...
router.post('/AddEntrantTelegramChannel', async (req: Request, res: Response) => {
    try {
        ...
        return res.status(200).json({
            message: 'EntrantTelegramChannel item successfully saved',
            data: savedEntrantTelegramChannel
        });
    } catch (error) {
...
router.put('/UpdateEntrantTelegramChannel/:id', async (req: Request, res: Response) => {
    try {
        ...
        const entrantTelegramChannel = await EntrantTelegramChannel.
        findByIdAndUpdate(req.params.id, req.body);

        const updatedEntrantTelegramChannel = await EntrantTelegramChannel.
        findById(req.params.id)

        // Respond with a success message and the updated entrantTelegramChannel item
        return res.status(200).json({'EntrantTelegramChannel item successfully updated':
          updatedEntrantTelegramChannel});
    } catch (error) {
        // Handle found errors
