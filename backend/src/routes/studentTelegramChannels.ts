...
router.put('/UpdateStudentTelegramChannel/:id', async (req: Request, res: Response) => {
  try{
    ...
    const studentTelegramChannel = await StudentTelegramChannel.findByIdAndUpdate(req.params.id, req.body);

    const updatedStudentTelegramChannel = await StudentTelegramChannel.findById(req.params.id)

    // Respond with a success message and the updated StudentTelegramChannel item
    return res.status(200).json({ 'StudentTelegramChannel item successfully updated': updatedStudentTelegramChannel });
  } catch (error) {
    // Handle found errors
